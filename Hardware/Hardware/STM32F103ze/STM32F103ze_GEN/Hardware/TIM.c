...
void TIM9_INIT(uint16_t ARR , uint16_t CCR)//SHI
{
	RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOB,ENABLE);
	GPIO_InitTypeDef GPIO_InitStructureTIM9;
	GPIO_InitStructureTIM9.GPIO_Mode = GPIO_Mode_AF_PP;
	GPIO_InitStructureTIM9.GPIO_Pin = GPIO_Pin_0;
	GPIO_InitStructureTIM9.GPIO_Speed = GPIO_Speed_50MHz;
	GPIO_Init(GPIOB, &GPIO_InitStructureTIM9);
  ...
  //TIM_SetAutoreload(TIMx,ARR);
  //TIM_SetCompare(TIMx,CCR);
	//while(TIM_GetFlagStatus(TIM9,TIM_FLAG_Update)==SET);
}
void TIM10_INIT(uint16_t ARR , uint16_t CCR)//SHI
{
	RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOB,ENABLE);
	GPIO_InitTypeDef GPIO_InitStructureTIM10;
	GPIO_InitStructureTIM10.GPIO_Mode = GPIO_Mode_AF_PP;
	GPIO_InitStructureTIM10.GPIO_Pin = GPIO_Pin_0;
	GPIO_InitStructureTIM10.GPIO_Speed = GPIO_Speed_50MHz;
	GPIO_Init(GPIOB, &GPIO_InitStructureTIM10);
  ...
  //TIM_SetAutoreload(TIMx,ARR);
  //TIM_SetCompare(TIMx,CCR);
	//while(TIM_GetFlagStatus(TIM10,TIM_FLAG_Update)==SET);
}
void SYSTICK(void)
{
	SysTick->LOAD = 250;
